...
while variPos == 0:
    if vh == "V":
        ...
        clear ()
        while int(posRow) > size - 1 or int(posCol) > size or int(posCol) < 1 or int(posRow) < 1:
            posRow = raw_input("In which row would you like to place your ship (Between 1 and " + str(size -1) + "): ")
            posCol = raw_input("In which column would you like to place your ship (Between 1 and " + str(size) + "): ")
            clear ()
        variPos = 1
    elif vh == "H":
        ...
        while int(posRow) > size or int(posCol) > size - 1 or int(posCol) < 1 or int(posRow) < 1:
            posRow = raw_input("In which row would you like to place your ship (Between 1 and " + str(size) + "): ")
            posCol = raw_input("In which column would you like to place your ship (Between 1 and " + str(size - 1) + "): ")
            clear ()
        variPos = 1
    else:
...
for turn in range(999):
    ...
    if (board[guess_row][guess_col] != "~"):
        # AI shouldn't ever guess where it has already. This is just an absolute backup incase something goes terribly wrong.
        print
        print '\033[31m' + "-- I guessed that one already, idiot, I am! --" + '\033[0m'
        print
        ...
        if gotOne == 0:
            ...
            print '\033[32m' + "-- My ship was located at: Row: " + str(ship_rowHum + 1) + " and Column: " + str(ship_colHum + 1) + "! --" + '\033[0m'
            print
            comwin = 1
    ...
    print "My guesses:"
    print_board(board)
    if (comwin == 1 & humwin == 1):
        print
        print '\033[32m' + "-- The game ended in a tie! A rematch is calling your name... --" + '\033[0m'
